...
With the prompt and the fifteen letters held exactly as in 01-full-grid, does luna at `medium` and at `xhigh` read every letter correctly every time, and which letters, if any, does it only sometimes get right?

## 2. Hypothesis, and where it came from

**The full marks of luna `medium` and luna `xhigh` in 01-full-grid were one lucky read each, not stability. Read ten times, the nine letters that every cell got right in 01 stay 10/10, and at least one of the six letters that missed somewhere in 01 misses again in at least one of the two cells.**
...
The hypothesis is confirmed if any of the six letters (`04`, `07`, `09`, `10`, `13`, `15`) misses at least once in either cell. It is refuted if both cells score 150/150. Either answer is useful: the first says 01's full cells are not to be trusted and the letters need work one by one; the second says these fifteen can be written down as stable on luna `medium` and the only step left is more reads.
...
The four variable files beside this report say exactly what ran: the same fifteen letters and the same prompt as 01-full-grid, two cells (luna `medium`, luna `xhigh`), ten reads per cell per letter. `diff 01-full-grid 02-ten-repeats` on the four files shows `cells.txt` and `repeats.txt` changed and nothing else.

terra was left out on cost: ten times the price per letter, and not the model the reading step currently uses.

Everything else was identical across the 300 calls: the same pages, the same prompt, the same request shape, four calls in flight.
...
| Letter | luna medium | luna xhigh | Wrong and confirmed |
|---|---|---|---|
| 01-electricity-bill | 10/10 | 10/10 | 0 |
| 02-gas-bill | 10/10 | 10/10 | 0 |
| 03-water-bill | 10/10 | 10/10 | 0 |
| 04-council-rates-notice | 10/10 | 10/10 | 0 |
| 05-animal-registration-overdue-notice | 10/10 | 10/10 | 0 |
| 06-parking-infringement-notice | 10/10 | 10/10 | 0 |
| 07-penalty-reminder-notice | **3/10** | **7/10** | 10 |
| 08-welfare-information-request | 10/10 | 10/10 | 0 |
| 09-specialist-account-statement | **6/10** | **9/10** | 3 |
| 10-private-health-annual-statement | **2/10** | **5/10** | 13 |
| 11-aged-care-monthly-statement | 10/10 | 10/10 | 0 |
| 12-failure-to-vote-notice | 10/10 | 10/10 | 0 |
| 13-product-recall-notice | 10/10 | 10/10 | 0 |
| 14-super-annual-member-statement | 10/10 | 10/10 | 0 |
| 15-insurance-key-facts-sheet | **8/10** | **9/10** | 3 |

A letter read 10/10 times bounds its per-read miss rate at 26% (95%, exact binomial).

300 calls, 300 valid replies.

**This run: 300 calls, $0.66 (A$0.92) in total.**
...
**Eleven letters were read right every time, in both cells.** The nine that every cell got right in 01 (`01`, `02`, `03`, `05`, `06`, `08`, `11`, `12`, `14`) stayed 10/10, and so did two of 01's six problem letters: `04-council-rates-notice`, which had missed in five of 01's twelve cells but never in luna `medium` or `xhigh`, and `13-product-recall-notice`, whose one miss in 01 was luna `none` doubling a digit. Twenty reads each without a miss is not proof, but it is the first evidence that those two belong with the eleven rather than with the four below.
...
**When a letter is wrong it is wrong the same way every time.** Each of the four has exactly one wrong answer, and every miss is that answer: `07` gives the second number printed on the page (`4797 2573 7796`) instead of the infringement number; `10` reports the `$167.43` printed on a statement that asks for nothing; `09` reports `$0.00` where the contract wants `No payment required`; `15` reports the generator's sample stamp `SYN-0025` as a reference. So the model is not inventing and is not noisy in general: on each of these pages it is choosing between two candidates it can see, and which one it picks changes from read to read. That is a selection error with a coin in it, not a locked one.

**More reasoning shifts the coin but does not remove it.** `xhigh` beat `medium` on every one of the four (7 against 3, 9 against 6, 5 against 2, 9 against 8 right out of ten) and cut wrong and confirmed from 20 to 9. It did not bring any of them to 10/10. 01's reading that effort does not order the cells was itself a one-read artefact; with ten reads, on these letters, it does.

**Twenty nine of the thirty one misses were `confirmed`.** The two that were not, `09`'s due date, would not have reached a screen. Every other miss would have.

**Cost per letter here is not the production cost.** 269 of the 300 calls were served from Azure's prompt cache, which is what repeating the same pages buys, and cached input is a tenth of the price. A letter in production is read once, uncached; 01's per-letter figures are the ones to quote.

## 6. Verdict

**The hypothesis is confirmed.** Four of the six letters that missed somewhere in 01 miss again in both luna cells, so the 15/15 that luna `medium` and luna `xhigh` scored in 01 were lucky reads, not stability. The nine letters predicted to stay clean did, and two more joined them.

What this settles: the fifteen letters split into eleven that are stable on luna at `medium` as far as ten reads can tell, and four that are not stable on luna at any effort tried. The four fail one way each, and three of the four ways were already named in 01: a second reference number on the page, an amount printed on a letter that asks for nothing, and the dataset's own stamp. The fourth, `$0.00` against `No payment required`, is a question about the contract's rule as much as about the model.
